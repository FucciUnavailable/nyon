...
def create(
    output: Path = typer.Option(
        Path("projects.json"), "--output", "-o", help="Output file path"
    ),
    auto_dates: bool = typer.Option(
        True, "--auto-dates", help="Auto-fill this week's dates"
    ),
):
    """
    ...
    try:
        ...
        if auto_dates:
            ...
            week_start = date.fromisoformat(Prompt.ask("Week start date (YYYY-MM-DD)"))
            week_end = date.fromisoformat(Prompt.ask("Week end date (YYYY-MM-DD)"))

        lead_name = Prompt.ask("Your name", default="Amine")
        team_name = Prompt.ask("Team name", default="PTR")
        ...
        milestone_date_str = Prompt.ask(
            "Milestone date (YYYY-MM-DD or leave empty)", default=""
        )
        next_milestone_date = (
            date.fromisoformat(milestone_date_str) if milestone_date_str else None
        )
        ...
        console.print(
            f"  python scripts/generate_weekly_report.py --input {output} --dry-run"
        )
...
def collect_projects() -> List[ProjectUpdate]:
    ...
    while True:
        ...
        status_choice = Prompt.ask(
            "Select status", choices=["1", "2", "3", "4"], default="1"
        )
        ...
        status_text = Prompt.ask(
            "Status description (optional)", default=status.display_name()
        )
...
def collect_summary_bullets(projects: List[ProjectUpdate]) -> List[str]:
    ...
    on_track = sum(
        1 for p in projects if p.status in [ProjectStatus.ON_TRACK, ProjectStatus.AHEAD]
    )
    ...
    while True:
        bullet = Prompt.ask("Add summary bullet (or press Enter to finish)", default="")
